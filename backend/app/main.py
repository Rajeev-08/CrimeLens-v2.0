...
news_api_key = os.getenv("NEWS_API_KEY") # <--- Add this to your .env if you have one
...
def get_map_context(payload: MapContextRequest):
    try:
        return {"amenities": get_nearby_amenities(payload.lat, payload.lon)}
    except:
        try:
            ...
            [out:json][timeout:10];
            (
              node["amenity"="police"](around:5000, {payload.lat}, {payload.lon});
              node["amenity"="hospital"](around:5000, {payload.lat}, {payload.lon});
            );
            out center;
            """
        ...
        return {"amenities": [
            {"lat": payload.lat + 0.01, "lon": payload.lon + 0.01, "name": "Central Station (Demo)", "type": "police"},
            {"lat": payload.lat - 0.01, "lon": payload.lon - 0.01, "name": "General Hospital (Demo)", "type": "hospital"}
        ]}
...
def generate_report_summary(payload: ReportRequest):
    if not model: return {"summary": "AI Summarizer not connected."}
    prompt = f"Analyst Brief: Area {payload.area}, {payload.total_crimes} crimes. Summarize risk."
    try:
        response = model.generate_content(prompt)
        return {"summary": response.text}
    except: return {"summary": "Summary generation failed."}

@app.post("/api/safety-assistant")
async def get_safety_tip(request: SafetyRequest):
    if not model: raise HTTPException(status_code=503, detail="AI Not Configured")
    async def stream():
        try:
            prompt = f"Context: {request.context}\nUser: {request.message}\nBrief Safety Tip:"
            response = model.generate_content(prompt, stream=True)
            for chunk in response:
                if chunk.text: yield chunk.text; await asyncio.sleep(0.01)
        except: yield "Service unavailable."
